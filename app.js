...
const renderTheatreLayout = (listOfUnavailableSeats = [], seatNoOffset = 1) => {
  ...
  for (let i = 0; i < 12; i++) {
    theatreSeats =
      theatreSeats +
      `<div id="booking-grid-${i + seatNoOffset}" class="grid-cell ${
        listOfUnavailableSeats.includes(i + seatNoOffset)
          ? "unavailable-seat"
          : "available-seat"
      }">${i + seatNoOffset}</div>`;
  }
  ...
  document
    ...
    .forEach((cell) => cell.addEventListener("click", onSeatClick));
};
...
function renderConfirmPurchaseForm() {
  const form = convertToHtmlDom(`
    <div id="confirm-purchase">
        <h3>Confirm your booking for seat numbers:${selectedSeats.join(
          ","
        )}</h3>
  ...
  document
    .querySelector("form")
    .addEventListener("submit", onPurchaseBtnClickHandler);
}
...
const renderMoviesList = async () => {
  ...
  const movieHolderElement = convertToHtmlDom(
    `<div class="movie-holder"></div>`
  );

  moviesList.forEach((movie) => {
    const movieElement =
      convertToHtmlDom(`<a class="movie-link" href="/${movie.name}">
